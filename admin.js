...
function loadPayrollSummary() {
    ...
    teachers.forEach(t => {
        ...
        html += `<div class="payroll-card"><h4>${t.name}</h4><div class="payroll-info"><p><strong>근무일수:</strong> ${days}일</p><p><strong>총 근무시간:</strong> ${Math.round(mins/60*10)/10}시간</p><p><strong>급여:</strong> ${PayrollCalculator.calculateSalary(mins,t.hourly_rate).toLocaleString()}원</p></div></div>`;
    });
    html += '</div>';
...
function downloadExcel() {
    ...
    teachers.forEach(t => {
        ...
        let mins = 0, days = 0;
        logs.forEach(l => { if (l.check_in_time && l.arrival_time) { mins += PayrollCalculator.calculateWorkMinutes(PayrollCalculator.adjustCheckInTime(l.check_in_time, l.first_bus_time), l.arrival_time); days++; } });
        csv += `${t.name},${days},${Math.round(mins/60*10)/10},${PayrollCalculator.calculateSalary(mins,t.hourly_rate)}\n`;
    });
...
function filterLogs() {
    ...
    tbody.innerHTML = logs.map(l => {
        ...
        if (l.check_in_time && l.arrival_time) {
            ...
            wh = Math.round(mins/60*10)/10 + '시간';
        }
